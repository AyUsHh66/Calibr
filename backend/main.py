...
async def get_plan_pdf(session_id: str, db: Session = Depends(get_session)): 
    ...
    try: 
        ...
        # Generate plan directly - skip LearningItem validation entirely 
        scores = json.loads(session.scores or '{}') 
        skills = json.loads(session.skills or '[]') 
        
        # Build PDF directly from raw data 
        buffer = io.BytesIO() 
        ...
        # Title 
        story.append(Paragraph("Calibr — Skill Assessment Report", 
                               styles['Title'])) 
        ...
        for skill in skills: 
            ...
            table_data.append([ 
                name, 
                str(score_obj.get('score', 'N/A')), 
                score_obj.get('level', 'N/A'), 
                score_obj.get('confidence_signal', 'N/A') 
            ]) 
        
        if len(table_data) > 1: 
            t = Table(table_data, 
                     colWidths=[2.5*inch, 1*inch, 1.5*inch, 1.5*inch]) 
        ...
        story.append(Paragraph( 
            "Generated by Calibr — AI Skill Assessment", 
            styles['Normal'] 
        )) 
        ...
        return Response( 
            ...
            headers={ 
                **cors_headers, 
                "Content-Disposition": f"attachment; filename=calibr-{session_id[:8]}.pdf", 
